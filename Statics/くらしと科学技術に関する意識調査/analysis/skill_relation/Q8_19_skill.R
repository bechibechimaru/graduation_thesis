...
# Q1で1または2を回答している人を抽出
q1_positive_data <- ra_data[ra_data$Q8_19 %in% c(1, 2), ]
# Q1で4または5を回答している人を抽出
q1_negative_data <- ra_data[ra_data$Q8_19 %in% c(4,5), ]
...
high_it_interest <- sum(it_positive_data$Q8_19 %in% c(1, 2)) / nrow(it_positive_data) * 100
# ITリテラシーが低い人の中で科学技術に関心がある人の割合
low_it_interest <- sum(it_negative_data$Q8_19 %in% c(1, 2)) / nrow(it_negative_data) * 100
...
contingency_table <- table(
  ...
  Interest = c(it_positive_data$Q8_19 %in% c(1, 2), it_negative_data$Q8_19 %in% c(1, 2))
)
...
# グラフの保存
ggsave("../../sources/skill_relation/Q8_19_IT_literacy_science_interest.png", p, width = 10, height = 6)
...
# グラフの保存
ggsave("../../sources/skill_relation/Q8_19_IT_literacy_science_interest_correlation.png", p2, width = 10, height = 6)

# 詳細な分析結果をファイルに出力
sink("../../sources/skill_relation/Q8_19_IT_literacy_science_interest_analysis.txt")
