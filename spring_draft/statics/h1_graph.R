...
modelsummary(
  models,
  title = "表1: 政府への信頼が各政策の重要度認識に与える影響（単純モデル）",
  stars = TRUE,
...
combined_models_filtered <- rbind(tidy_model_1)
...
ggsave("h1_graph.png", revised_plot, width = 10, height = 6)
print("グラフを保存しました")
...
bar_data <- analysis_data %>%
  ...
  # グループごとに集計
  summarise(
    # is_important_q8_5 の平均値を計算 (これが割合になる)
    proportion_important = mean(is_important_q8_5, na.rm = TRUE)
  ) %>%
...
# 3. グラフの表示と保存
print(bar_plot)
ggsave("h1_bar_chart.png", plot = bar_plot, width = 8, height = 6)
print("棒グラフを 'h1_bar_chart.png' として保存しました。")
